importing('currentDate','../../plugin/bootstrap/bootstrap.3.31.css');
define([
    'underscore',
    'echarts',
    ...
    '../dat/fstPage.js'], function (_, ec,
                                    fstPageTpl, newListTpl, noticeListTpl, ruleListTpl, tableDownListTpl, daibanListTpl, noticeOneTpl, ruleOneTpl,
                                    fstPageAjax) {
    ...
    return {
        showFstPage: function () {
            ...
            //显示各处项目开支情况图
            if (top.opsMap && top.opsMap.get('office_exes') && top.opsMap.get('office_exes') == 1) {
                _self.getDevelopStat();
            }
            //显示各处物品申领情况图
            if (top.opsMap && top.opsMap.get('office_item') && top.opsMap.get('office_item') == 1) {
                _self.getGoodsStat();
            }
            //显示各处用车情况图
            if (top.opsMap && top.opsMap.get('office_car') && top.opsMap.get('office_car') == 1) {
                _self.getCarsStat();
            }
